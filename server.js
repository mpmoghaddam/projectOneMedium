...
const { compare, chooseNumberOfGame, checkIfGameOver, startGame, getCountP2, getCountP1, getNumberOfPlays } = require("./functions");
...
app.get('/player2Name', async (req, res) => {
  ...
  player2 = name;
  // let game = await createGame ({player2Name:name})
...
app.get('/numberOfPlays', async (req, res) => {
  ...
  gameId = req.query.gameId;
  let game = await updateGameById(gameId, {totalnumberOfPlays: number})
...
app.get('/gameFinalResult', async (req, res) => {
  ...
  let game = await updateGameById(gameId, {playaer1TotalWin: countp1},
    {player2TotalWin: countp2}, {totalNumberOfPlays: numberOfPlays})
    res.send(winnerAll);
});
